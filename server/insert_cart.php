...
if ($resultstock->num_rows > 0) {
     while ($row = $resultstock ->fetch_assoc()){
        ...
        if ($quantity == 0) { //product is out of stock
           ...
           echo $sqlcheckcart = "SELECT * FROM tbl_cart WHERE prid = '$prid' AND email = '$email'"; //check if the product is already in cart
            $resultcart = $conn->query($sqlcheckcart);
            if ($resultcart->num_rows == 0) {//product is not in the cart proceed with insert new
                 echo $sqladdtocart = "INSERT INTO tbl_cart (email, prid, cartqty) VALUES ('$email','$prid','1')";
                ...
                echo $sqlupdatecart = "UPDATE tbl_cart SET cartqty = cartqty +1 WHERE prid = '$prid' AND email = '$email'";
